	...
	matrix3x4_t *matrix)
{
	...
	if (is_player) {
		...
		if (ent && g_LocalPlayer && ent->IsAlive()) {
			...
			if (g_Options.chams_player_enabled || PlayerList[ent->EntIndex()].bFriend)
			{
				if (ent && g_LocalPlayer && ent->IsAlive()) {
					const auto enemy = ent->m_iTeamNum() != g_LocalPlayer->m_iTeamNum();
					if (!enemy && g_Options.chams_player_enemies_only)
					...
					Color clr_front;
					if (PlayerList[ent->EntIndex()].bFriend)
					{
						clr_front = Color(255, 255, 255);
					}
					else
					{
						clr_front = enemy ? g_Options.color_chams_player_enemy_visible : g_Options.color_chams_player_ally_visible;
					}
