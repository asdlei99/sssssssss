...
double SMG::GetAngleDifference(double alpha, double beta)
{
...
QAngle SMG::SmoothAngle(QAngle angles, QAngle destination, double x, double y)
{
...
QAngle SMG::CalculateAngle(Vector target)
{
	QAngle angle = QAngle();
...
int SMG::GetHitbox(C_BasePlayer* player)
{
...
void SMG::OnCreateMove(CUserCmd * cmd)
{
	...
	if (target)
	{
		if (IsValid(target))
		{
			QAngle angle = SmoothAngle(cmd->viewangles, CalculateAngle(target->GetHitboxPos(GetHitbox(target))), g_Options.smg_smooth_x, g_Options.smg_smooth_y);

			angle.normalize();

			if (!g_Options.smg_silent)
			{
...
bool SMG::IsValid(C_BasePlayer* entity)
{
	if (!entity)
		return false;
	...
	if (entity->m_bGunGameImmunity())
		return false;
	if (entity->m_iTeamNum() == g_LocalPlayer->m_iTeamNum() && !g_Options.smg_target_team)
		return false;
	if (entity->IsDormant())
		return false;
...
int SMG::GetClosest(CUserCmd* cmd)
{
	float curDist = -1;
	int closest = -1;
	...
	for (auto i = 0; i < g_GlobalVars->maxClients; i++)
	{
		...
		if (dif < curDist || curDist == -1 && dif < g_Options.smg_fov)
		{
			curDist = dif;
			closest = i;
		}
