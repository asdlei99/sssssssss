...
double Pistol::GetAngleDifference(double alpha, double beta)
{
...
QAngle Pistol::SmoothAngle(QAngle angles, QAngle destination, double x, double y)
{
...
QAngle Pistol::CalculateAngle(Vector target)
{
	QAngle angle = QAngle();
...
int Pistol::GetHitbox(C_BasePlayer* player)
{
...
void Pistol::OnCreateMove(CUserCmd * cmd)
{
	...
	if (target)
	{
		if (IsValid(target))
		{
			QAngle angle = SmoothAngle(cmd->viewangles, CalculateAngle(target->GetHitboxPos(GetHitbox(target))), g_Options.pistol_smooth_x, g_Options.pistol_smooth_y);

			angle.normalize();

			if (!g_Options.pistol_silent)
			{
...
bool Pistol::IsValid(C_BasePlayer* entity)
{
	if (!entity)
		return false;
	...
	if (entity->m_bGunGameImmunity())
		return false;
	if (entity->m_iTeamNum() == g_LocalPlayer->m_iTeamNum() && !g_Options.pistol_target_team)
		return false;
	if (entity->IsDormant())
		return false;
...
int Pistol::GetClosest(CUserCmd* cmd)
{
	float curDist = -1;
	int closest = -1;
	...
	for (auto i = 0; i < g_GlobalVars->maxClients; i++)
	{
		...
		if (dif < curDist || curDist == -1 && dif < g_Options.pistol_fov)
		{
			curDist = dif;
			closest = i;
		}
