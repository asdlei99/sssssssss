...
double Sniper::GetAngleDifference(double alpha, double beta)
{
...
QAngle Sniper::SmoothAngle(QAngle angles, QAngle destination, double x, double y)
{
...
QAngle Sniper::CalculateAngle(Vector target)
{
	QAngle angle = QAngle();
...
int Sniper::GetHitbox(C_BasePlayer* player)
{
...
void Sniper::OnCreateMove(CUserCmd * cmd)
{
	...
	if (target)
	{
		if (IsValid(target))
		{
			QAngle angle = SmoothAngle(cmd->viewangles, CalculateAngle(target->GetHitboxPos(GetHitbox(target))), g_Options.sniper_smooth_x, g_Options.sniper_smooth_y);

			angle.normalize();

			if (!g_Options.sniper_silent)
			{
...
bool Sniper::IsValid(C_BasePlayer* entity)
{
	if (!entity)
		return false;
	...
	if (entity->m_bGunGameImmunity())
		return false;
	if (entity->m_iTeamNum() == g_LocalPlayer->m_iTeamNum() && !g_Options.sniper_target_team)
		return false;
	if (entity->IsDormant())
		return false;
...
int Sniper::GetClosest(CUserCmd* cmd)
{
	float curDist = -1;
	int closest = -1;
	...
	for (auto i = 0; i < g_GlobalVars->maxClients; i++)
	{
		...
		if (dif < curDist || curDist == -1 && dif < g_Options.sniper_fov)
		{
			curDist = dif;
			closest = i;
		}
