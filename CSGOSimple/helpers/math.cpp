...
namespace Math
{
	...
	float GetFov(const QAngle& viewAngle, const QAngle& aimAngle)
	{
		...
		AngleVectorsAV(viewAngle, aim);
		AngleVectorsAV(aimAngle, ang);
	...
	{
		return (float)sqrt(pow((enemyCoords.x - myCoords.x), 2.0) + pow((enemyCoords.y - myCoords.y), 2.0) + pow((enemyCoords.z - myCoords.z), 2.0));
	}
