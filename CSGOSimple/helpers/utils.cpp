...
namespace Utils
{
	...
	*/
	...
	/*
	* @brief Play the specified sound by executing a console command
	*
	* @param sound Sound
	*/
	void ConsoleCommandSound(char* sound)
	{
		char buf[128];
		sprintf(buf, "play %s", sound);
		g_EngineClient->ClientCmd_Unrestricted(buf, 0);
	}
}